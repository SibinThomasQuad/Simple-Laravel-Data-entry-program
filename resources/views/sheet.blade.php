...
<tr>
  ...
  ?>
  <td><input onkeyup="flash('{{$key}}',this.value);" value='{{$content}}' type='text' name='n{{$i}}-{{$j}}'></td>
  <?php
  }
  ?>
</tr>
...
xhttp.open("GET", "{{url('/live-save')}}?id={{$data->id}}&location="+location+"&content="+content, true);
xhttp.send();
}
function addrow()
{
...
xhttp.open("GET", "{{url('/add')}}?id={{$data->id}}&curve=0", true);
xhttp.send();
}
function addcol()
{
...
xhttp.open("GET", "{{url('/add')}}?id={{$data->id}}&curve=1", true);
xhttp.send();
}
function get_count_e(type_request)
{
    ...
    xhttp.onreadystatechange = function() {
      ...
      xhttp.open("GET", "{{url('/count')}}?id={{$data->id}}&curve="+type_request, true);
      xhttp.send();
      return 2;
}
</script>
